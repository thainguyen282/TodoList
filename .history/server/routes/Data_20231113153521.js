...
router.put('/api/data/655285310ba7219d1bb06276', async (req, res) => {
    try{
        const existingDataItem = await DataItemModel.findById(req.params.id);
        const updateDataItem = await DataItemModel.findByIdAndUpdate(req.params.id, {done: existingItem.done === true ? false : true})
