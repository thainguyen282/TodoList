...
function Home() {
    ...
    const handleStatEdit = async (id) => {
        try {
            // Make the PUT request to update the item on the server
            const { data: updatedData } = await axios.put(`http://localhost:5500/api/stat/655285310ba7219d1bb06276`, {
                // Include the data you want to update on the server
                // If newPercent, newTotal, and newNumCheck aren't available here, ensure they are declared before the try block
                percent: newPercent,
                total: newTotal, 
    ...
    return (
        <div className="home">
            ...
            <div> 
                <button type="button" onClick={handleStatEdit}>Done!</button>
            </div>
        </div>
    )
}
